...
img = cv2.imread(
    'C:/Users/yukimura/Documents/Workplace/FFT_audio/squares.png')
img = rgb2gray(img)
ax[0].imshow(img)

imgf = fft2(img)
imgjason = ImgFFTJason(img)
imgyuki = ImgFFTYuki(img)
ax[1].imshow(np.abs(fftshift(imgf)))

rec = ifft2(imgf)
ax[2].imshow(np.abs(rec))
ax[3].imshow(np.abs(fftshift(imgyuki)))
ax[4].imshow(np.abs(fftshift(imgjason)))
